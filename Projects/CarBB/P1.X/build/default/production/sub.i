...
void store_data(void)
{
    ...
    if(lap == 10)
    {
        ...
        {
            char temp = read_external_eeprom(k);
            write_external_eeprom(k-5, temp);
        }
    }
    ...
    for(int i=0;i<5;i++)
    {
        write_external_eeprom(i+(lap * 5), store[i]);
    }
    lap++;
}
...
void menu(void)
{
   ...
   clcd_print(menu_data[ menu_index], (0x80 + (1)));
   clcd_print(menu_data[ menu_index+1], (0xC0 + (1)));
